...
public extension Font {
    ...
    enum Resource {
        ...
        case path(String)
        /// Load a font by its PostScript name
...
internal extension Font {
    func load() -> Loaded {
        ...
        case .postScriptName(let name):
            return load(withPostScriptName: name) ?? loadDefaultFont()
        }
    }
    ...
    private func load(withPostScriptName postScriptName: String) -> Loaded? {
        guard let font = CGFont((postScriptName as CFString)) else { return nil }
        return CTFontCreateWithGraphicsFont(font, CGFloat(size), nil, nil)
    }
}
